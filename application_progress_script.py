...
@click.command()
@click.option('--fund_id', prompt='Enter fund_id: ',
              help='Provide fund_id from the fund.')
@click.option('--round_id', prompt='Enter round_id: ',
              help='Provide fund_id from the fund.')
def send_email_on_deadline_task(fund_id, round_id):
   

    current_date_time = datetime.now(ZoneInfo("Europe/London")).replace(microsecond=0).strftime("%Y-%m-%d %H:%M:%S")
